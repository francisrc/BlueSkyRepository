...
namespace BSky.Statistics.R
{
    public class RCommandStrings
    {
        ...
        public static string SaveDatasetToFile(ServerDataSource dataSource)//string fullpathfilename, string filetype, string datasetName)
        {
            ...
            string worksheetname = null;
            if (filetype.Equals("XLS") || filetype.Equals("XLSX")) worksheetname = "Sheet1";

            // set some extra parameter here if needed. Like Excel sheetname. replace etc..
            if (!filetype.Equals("SAV"))
                return string.Format("UAsaveDataset(fullpathfilename=\"{0}\",filetype='{1}', newWorksheetName='{2}', dataSetNameOrIndex='{3}')", FormatFileName(fullpathfilename), filetype, worksheetname, datasetname);
            else
                return "";
        }//Anil #5
        ...
        public static string closeDataset(ServerDataSource dataSource)
        {
            ...
            //string fullpathfilename = Path.Combine(dirPath, fileName);// dirPath + "\\" + fileName;
            string datasetname = dataSource.Name;
            return string.Format("UAcloseDataset('{0}')", datasetname);
        }//Anil #5
        private static string LoadDataSource2(ServerDataSource dataSource) { return string.Format("{0}<-uaopendataset({1}, '{2}', {3}, '{4}')", dataSource.Name, (uint)dataSource.DataSetType, FormatFileName(dataSource.FileNameWithPath), dataSource.HasHeader.ToString().ToUpper(), dataSource.FieldSperator); }
        ...
        public static string AddNewDatagridCol(string colName, string dgridval, int rowindex, ServerDataSource dataSource) { return string.Format("BSkyAddVarRow('{0}', '{1}', {2}, '{3}')",  colName, dgridval, rowindex, dataSource.Name); }//add new col in datagrid and new row in var grid
