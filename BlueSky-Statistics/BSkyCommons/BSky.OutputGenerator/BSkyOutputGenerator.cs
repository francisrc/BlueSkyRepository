...
namespace BSky.OutputGenerator
{
    public class BSkyOutputGenerator
    {
        ...
        public List<SessionOutput> GenerateOutput(string fullpathzipfilename)//12Sep2012
        {
            ...
            {
                MessageBox.Show("BSky Ouput File not Found! ");
                return null;
            }
        ...
        public List<SessionOutput> html2xml(string fullpathfilename)
        {
            ...
            {
                MessageBox.Show("Output File is not a BSky Format! ");
                return null;
            }

            string r = Regex.Replace(s, @"<head>", "");
            string t = r;// Regex.Replace(r, @"<style[\S|\s]*</style>", "");
            ...
            for (int j = 0; j < anamc.Count; j++)///loop on number of analysis
            {
                ...
                //lst.NameOfAnalysis = nameofanalysis;//For Parent Node name 02Aug2012
                anaStart = anamc[j].Index;
                anaEnd = anamc2[j].Index;
                string oneAnalysis = onesession.Substring(anaStart, anaEnd - anaStart + 15);
                anaXd.LoadXml(oneAnalysis);  //one analysis xml doc. Use it for getting headers etc..  
                ...
                foreach (XmlNode ln in anaXd.ChildNodes.Item(0).ChildNodes)
                {
                    ...
                    switch (ln.Name.Trim())
                    {
                        ...
                        case "aup":
                            #region AUPara
                            AUP = createAUPara(ln, "", ""); //29Oct2013 controltype will be populated in createAUPara()
                            if (AUP != null)
                                lst.Add(AUP);
                        ...
                        case "bskynotes":
                            #region BSkyNotes
                            if (noofnotes >= 1)//for maintaining the sequence, this is in middle of auparas
                            {
                                BSkyNotes bsnotes = createBSkyNotes(ln, "", "Notes"); //createBSkyNotes(anaXd, "bskyanalysis/bskynotes[" + notesno + "]", "Notes");
                                if (bsnotes != null)
                                    lst.Add(bsnotes);
                            }
                            #endregion
                        ...
                        case "graphic":
                            #region Graphic
                            //if (anaXd.SelectNodes("bskyanalysis/graphic").Count == 1)//graphic image exists 11Sep2012
                            //{
                                string tempDir = System.IO.Path.GetDirectoryName(fullpathfilename);//C:\Users\Anil\AppData\Local\Temp\
                        ...
                        case "auxgrid":
                            #region FlexGrid
                            ///////////// find C1Flexgrid Generate it //////S/////
                            //// 1.Find C1FlexGrid in html
                            //// 2.Create XML 
                            //// 3.Generate C1FlexGrid for rendering
                            ...
                            //anagrid.LoadXml(v.Substring(aux1[0].Index, aux2[0].Index - aux1[0].Index + 10));
                            ...
                            for (int k = 0; k < mc.Count; k++) /// loop till all C1FlexGrid in current Analysis get generated
                            {
                                ...
                                //mypanel.Children.Add(xgrid); // adding grid to outputviewer
                ...
                ////// one analysis ends so now create treeview for that ////
                ///// lst must only contain one analysis ////
                //PopulateTree(lst);
                allanalysis.Add(lst);
            }
            ...
            #endregion
            //outputDataList.Add(analysisdata);/// can be used to populate currently running output window.
            //return allanalysis;
            return allSessions;
        }
        ...
        public void html2flex(XmlDocument xd, AUGrid c1FlexGrid1)
        {
            ...
            //double NoOfRows = rowheaders.GetLength(0);
            //double EachRowHeight = 22; //pixel
        ...
        /// Create AUParagraph
        /// </summary>
        /// <param name="xd">Xml Doc that contains AUParagraph info</param>
        /// <param name="selectNode">Node that is to be selected for creating AUPara</param>
        /// <param name="controType">Name of this AUParagraph</param>
        /// <returns></returns>
        ...
        public BSkyNotes createBSkyNotes(XmlNode xd, string selectNode, string controType)//06Nov2012
        {
            ...
            xn = xd.SelectSingleNode("collapsetext");// (selectNode + "/collapsetext");
            bsn.CollapsedText = (xn != null) ? xn.InnerText.Trim() : "";

            xn = xd.SelectSingleNode("showrow");//(selectNode + "/showrow");
            bsn.ShowRow_Index = int.Parse((xn != null) ? xn.InnerText.Trim() : "1");

            xn = xd.SelectSingleNode("splitposi");// (selectNode + "/splitposi");
            bsn.NotesSplitPosition = uint.Parse((xn != null) ? xn.InnerText.Trim() : "1"); bsn.RightPart = 6; bsn.LeftPart = 1;

            xn = xd.SelectSingleNode("notesheading");// (selectNode + "/notesheading");
            bsn.HearderText = (xn != null) ? xn.InnerText.Trim() : "";

            XmlNodeList xnlrow = xd.SelectNodes("notesrow");// (selectNode + "/notesrow");
            XmlNodeList xnlcol = xd.SelectNodes("notesrow[1]/notescol");// (selectNode + "/notesrow[1]/notescol");
            if (xnlrow == null || xnlcol == null || xnlcol.Count < 1 || xnlrow.Count < 1) //09jul2013 if there is no notes tags in xml doc
                return null;
            ...
            for (int i = 0; i < rowcount; i++)
            {
                ...
                {
                    xn = xd.SelectSingleNode("notesrow[" + (i + 1) + "]/notescol[" + (j + 1) + "]");// (selectNode + "/notesrow[" + (i + 1) + "]/notescol[" + (j + 1) + "]");
