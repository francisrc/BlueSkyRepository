                    ...
                    if (output.HasValue && output.Value)
                    {
                        ...
                        if (!string.IsNullOrEmpty(xamlFile) && !string.IsNullOrEmpty(outputFile))
                        {
                            ...
                            try
                            {
                                ...
                                bool? result = dashboardService.SetElementLocaton(menulocation, title, Path.Combine(string.Format(@"{0}", BSkyAppData.BSkyDataDirConfigBkSlash), xamlFile), false, "");//23Apr2015 
                                ...
                                if (result.HasValue && (result.Value || msgResult == MessageBoxResult.Yes))
                                {
                                    ...
                                    canvas.OutputDefinition = Path.GetFullPath(string.Format(@"{0}", BSkyAppData.BSkyDataDirConfigBkSlash) + outputFile);//23Apr2015
                                    ...
                                    FileStream outputstream = System.IO.File.Create(string.Format(@"{0}", BSkyAppData.BSkyDataDirConfigBkSlash) + xamlFile);//23Apr2015 
                                ...
                                else
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(location))
                                    {
                                        ...
                                        dashboardService.SetElementLocaton(location, sInput, Path.Combine(string.Format(@"{0}", BSkyAppData.BSkyDataDirConfigBkSlash), sInput + xamlFile), true, "");//23Apr2015
                                        ...
                                        canvas.OutputDefinition = Path.GetFullPath(string.Format(@"{0}", BSkyAppData.BSkyDataDirConfigBkSlash) + sInput + xamlFile.Replace("xaml", "xml"));//23Apr2015 //output template filename will be same as dialog filename. No matter whats in zip file.
                                        System.IO.File.Copy(Path.Combine(tempDir, outputFile), canvas.OutputDefinition, true);
                                        ...
                                        string installpath = Path.GetFullPath(string.Format(@"{0}", BSkyAppData.BSkyDataDirConfigBkSlash) + sInput + xamlFile);//23Apr2015
